...
class BinaryTree {
   ...
   friend void  visit(BinaryTreeNode<int> *tem,string str);
   friend void Store(BinaryTreeNode<int>* u);
   public:
      BinaryTree() {root = 0;};
      ~BinaryTree(){}; 
      bool IsEmpty() const
      ...
      void PreOutput() {PreOrder(Output, root); std::cout << endl;}
      void InOutput() {InOrder(Output, root); cout << endl;}
      void PostOutput() {PostOrder(Output, root); std::cout << endl;}
//	  void PostStore() {PostOrder(Store, root); std::cout << endl;}
